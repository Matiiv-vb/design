...
export default defineComponent({
  ...
  setup(): SetupData {
    ...
    const { value: addressLine, errorMessage: errorMessageAddressLine } =
      useField("addressLine", validate);
    const { value: streetNumber, errorMessage: errorMessageStreetNumber } =
      useField("streetNumber", validate);
    const { value: town, errorMessage: errorMessageTown } = useField(
      "town",
      validate
    );
    ...
    const fieldValidationClass = (type: string): string | null => {
      ...
      return addressErrorMessage[type] === undefined
        ? "is-valid"
        : "is-invalid";
    };
